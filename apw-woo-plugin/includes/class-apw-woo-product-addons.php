...
function apw_woo_get_product_addons($product)
{
    ...
    // Make sure Product Add-ons is active and function exists
    if (!function_exists('get_product_addons')) {
        if (APW_WOO_DEBUG_MODE && function_exists('apw_woo_log')) {
            apw_woo_log('Product Add-ons function get_product_addons() not available');
        }
        return array();
    }
    ...
    if (APW_WOO_DEBUG_MODE && function_exists('apw_woo_log')) {
        $addon_count = is_array($addons) ? count($addons) : 0;
        apw_woo_log(sprintf('Found %d product add-on groups for product #%d', $addon_count, $product_id));
    }

    return is_array($addons) ? $addons : array(); // Ensure return is always an array
}
...
function apw_woo_get_product_addons_html($product, $args = array())
{
    ...
    // Make sure Product Add-ons display class/method exists
...
//{
//    // Check if Product Add-ons is active using the helper function
...
//    $addons_class_file = APW_WOO_PLUGIN_DIR . 'includes/class-apw-woo-product-addons.php';
//    if (file_exists($addons_class_file)) {
//        require_once $addons_class_file;
//        // Initialize the class only if it exists after include
...
//        } else {
//            if (APW_WOO_DEBUG_MODE && function_exists('apw_woo_log')) {
//                apw_woo_log('APW_Woo_Product_Addons class not found after including file.', 'warning');
//            }
//        }
//    } else {
//        if (APW_WOO_DEBUG_MODE && function_exists('apw_woo_log')) {
//            apw_woo_log('Product Add-ons class file not found: ' . $addons_class_file, 'warning');
//        }
//    }
//}

// REMOVED INCORRECT FUNCTION DEFINITION BELOW

// function apw_woo_initialize_intuit_integration() { ... } // This was removed
