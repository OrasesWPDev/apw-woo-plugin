 ...
 * Plugin constants
 */
define('APW_WOO_VERSION', '1.23.23');
...
function apw_woo_autoload_files()
{
    ...
    foreach ($class_files as $file) {
        ...
        // Skip any old updater files that may still exist on servers
        if (in_array($basename, ['class-apw-woo-github-updater.php', 'class-apw-woo-updater.php'])) {
            apw_woo_log('Skipping deprecated updater file: ' . $basename);
            continue;
        }
        if (file_exists($file) && !isset($loaded_files[$file])) {
            require_once $file;
