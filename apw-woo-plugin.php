...
function apw_woo_autoload_files()
{
    ...
    foreach ($class_files as $file) {
        ...
        // Skip any old updater files that may still exist
        if ($basename === 'class-apw-woo-simple-updater.php') {
            apw_woo_log('Skipping old updater file: ' . $basename);
            continue;
        }
        if (file_exists($file) && !isset($loaded_files[$file])) {
            require_once $file;
